...
import { LiteEngine } from './engines';
...
const lookupBlockByID = async (engine: LiteEngine, props: { seqno: number, shard: string, workchain: number }, queryArgs?: QueryArgs) => {
  return engine.query(Functions.liteServer_lookupBlock, {
...
const getBlockHeader = async (engine: LiteEngine, props: BlockID, queryArgs?: QueryArgs) => {
  return engine.query(Functions.liteServer_getBlockHeader, {
...
function getCacheMap(mapKind: MapKind, mapOptions?: number | ((mapKind: MapKind) => CacheMap)): CacheMap {
  ...
  return new LRUMap(defaultOpts.cacheSize);
}

export class LiteClient {
  readonly engine: LiteEngine;
  private blockLockup: DataLoader<BlockLookupRequest, liteServer_blockHeader, string>;
  private shardsLockup: DataLoader<BlockID, AllShardsResponse, string>;
  private blockHeader: DataLoader<BlockID, liteServer_blockHeader, string>;
  private accounts: DataLoader<AccountsDataLoaderKey, ClientAccountState, string>;
  ...
  constructor(opts: {
    ...
    let batchSize = typeof opts.batchSize === 'number' ? opts.batchSize : defaultOpts.batchSize;

    this.blockLockup = new DataLoader(async (s) => {
      return Promise.all(s.map(async (v) => {
        if (v.mode === 'utime') {
    ...
    this.blockHeader = new DataLoader(async (s) => {
      return Promise.all(s.map(async (v) => getBlockHeader(this.engine, v)));
    }, {
      maxBatchSize: batchSize,
    ...
    this.shardsLockup = new DataLoader<BlockID, AllShardsResponse, string>(async (s) => {
      return Promise.all(s.map(async (v) => getAllShardsInfo(this.engine, v)));
    }, {
      maxBatchSize: batchSize,
    ...
    this.accounts = new DataLoader<AccountsDataLoaderKey, ClientAccountState, string>(async (s) => {
      return Promise.all(s.map(async (v) => this.getAccountStateRaw(v.address, {
        fileHash: v.fileHash,
        rootHash: v.rootHash,
        seqno: v.seqno,
        shard: v.shard,
  ...
  getVersion = async (queryArgs?: QueryArgs) => {
    return (this.engine.query(Functions.liteServer_getVersion, { kind: 'liteServer.getVersion' }, queryArgs));
  };
  ...
  getAccountTransaction = async (src: Address, lt: string, block: BlockID, queryArgs?: QueryArgs) => {
    return this.engine.query(Functions.liteServer_getOneTransaction, {
  ...
  listBlockTransactions = async (block: BlockID, args?: {
    ...
    return this.engine.query(Functions.liteServer_listBlockTransactions, {
